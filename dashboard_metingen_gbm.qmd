...
dateRangeInput("periode", "Periode", start = datum_begin, language = "nl", separator = " t/m ", format = "dd-mm-yyyy" )
```
...
```{r}
#| title: "Aangetroffen gewasbeschermingsmiddelen"
...
gbm_recent <- reactive({
  ...
  filter(parnr > 999, parnr < 2000,
         datum >= input$periode[[1]],
         datum <= input$periode[[2]]) %>%
...
# Update inputs
datum_begin <- floor_date(Sys.Date() %m-% period(80, "days"), unit = "months")
updateDateRangeInput(session = session, inputId = "periode", start = datum_begin)
...
gbm_recent_filtered <- reactive({
  gbm_recent_filtered <- gbm_recent()
  
  if (mp_sel_kaart() != "") gbm_recent_filtered <- filter(gbm_recent_filtered, mp == mp_sel_kaart())
  
  gbm_recent_filtered
})

output$tabel_recent <- renderReactable({tabel_recent(gbm_recent_filtered())})
...
mp_sel_tabel <- reactive({
  ...
  if (is.null(getReactableState("tabel_recent")$selected)) {
    ...
    mp_selected <-
    gbm_recent_filtered()[getReactableState("tabel_recent")$selected,] %>%
    pull(mp)
  }
